...
const rawOpenVPNCoreConfigSchema = z
    ...
    .catchall(jsonValueSchema);
const REQUIRED_PKI_FIELDS = ["ca_cert", "server_cert", "server_key", "tls_crypt_key"];
...
export function isValidCidr(value) {
    ...
    if (!address || !prefixRaw || !/^\d{1,3}$/.test(prefixRaw))
        return false;
...
export function validateOpenVPNCoreConfig(input) {
    const parsed = rawOpenVPNCoreConfigSchema.safeParse(input);
    if (!parsed.success) {
        return {
            ok: false,
            issues: parsed.error.issues.map(zodIssue => issue(pathForZod(zodIssue.path.filter((part) => typeof part === "string" || typeof part === "number")), "OV_SCHEMA_INVALID_CORE_CONFIG", zodIssue.message))
        };
    }
    const issues = [];
    let config;
...
export function assertValidOpenVPNCoreConfig(input) {
...
export function isOpenVPNInstance(value) {
    return !!value && typeof value === "object" && typeof value.tag === "string" && value.protocol !== undefined;
}
